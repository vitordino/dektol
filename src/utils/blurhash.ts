...
type CanvasOptions = { size: number; quality: number }

const DEFAULT_OPTIONS = { size: 16, quality: 40 }
...
const generateBlurhashStringFromUrl = async (url: string) => {
...
const generateImageURIFromHash = async (
  ...
  const raw = { channels: 4, width: hashWidth, height: hashHeight } as const
